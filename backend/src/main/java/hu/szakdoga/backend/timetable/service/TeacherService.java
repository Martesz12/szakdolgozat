...
public class TeacherService {
    ...
    public void deleteTeacher(Long id) {
        ...
        if (!exist) throw new IllegalStateException("Teacher with id " + id + " does not exist.");
        teacherRepository.deleteById(id);
    }

    public TeacherEntity convertDtoToEntity(TeacherDTO dto) {
        UserEntity user = userRepository.findById(dto.userId).orElseThrow(() -> new EntityNotFoundException("User by id " + dto.userId + " was not found."));

        return new TeacherEntity(dto.id, dto.name, dto.webpage, dto.email, user, dto.office, dto.moreInformation);
    }

    public TeacherDTO convertEntityToDto(TeacherEntity entity) {
        return new TeacherDTO(entity.getId(), entity.getName(), entity.getWebpage(), entity.getEmail(), entity.getUser().getId(), entity.getOffice(), entity.getMoreInformation());
    }
}
